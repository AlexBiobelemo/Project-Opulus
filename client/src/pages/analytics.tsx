...
export default function Analytics() {
  const { data: stats, isLoading: statsLoading, error: statsError } = useQuery<SimulationStats>({
    queryKey: ['/api/stats'],
    refetchInterval: 2000,
  });

  const { data: bots = [], isLoading: botsLoading, error: botsError } = useQuery<Bot[]>({
    queryKey: ['/api/bots'],
  ...
  const isLoading = statsLoading || botsLoading || postsLoading;
  const hasError = statsError || botsError || postsError;

  if (isLoading) {
    return (
      <Layout title="Analytics" description="Simulation performance metrics and insights">
        <div className="p-6">
